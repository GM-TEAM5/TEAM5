...
public class PlayerDataSO : ScriptableObject
{
    ...
    public void OnStageClear(Player player)
    {
        ...
        // Dictionary의 Values 대신 원래 키 순서대로 스킬 저장
        skills = new List<SkillItemSO>();
        foreach (KeyCode key in PlayerInputManager.Instance.skillKeys)
        {
            if (player.skills.ContainsKey(key))
            {
                skills.Add(player.skills[key].skillData);
            }
        }
